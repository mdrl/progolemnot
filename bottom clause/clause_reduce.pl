...
:- module(clause_reduce,
            [
              ...
              negative_clause_reduce_with_neg/8,
...
:- use_module('../utils/clause', [atomArgs/4, split_pos_neg/6]).
...
fail_profile_to_weight_balance(A, [], B, _, []):- A < 0, B < 0, !.
...
negative_clause_reduce_with_neg(Clause, ClauseSignature, PosEIDs, NegEIDs,
                       ReducedClause, ReducedClauseSig, FPosEIDsCov, FNegEIDsCov):-
  split_pos_neg(Clause, ClauseSignature, PosClause, PosClauseSig, NegClause, NegClauseSig),
  full_hypothesis_coverage(Clause, ClauseSignature, PosEIDs, NegEIDs, PosEIDsCov, NegEIDsCov),
  %% negative_clause_reduce(PosClause, PosClauseSig, PosEIDs, NegEIDs, PosEIDsCov, NegEIDsCov,
  %%                        ReducedPosClause, ReducedPosClauseSig, TFPosEIDsCov, TFNegEIDsCov),
  negative_clause_reduce(Clause, ClauseSig, PosEIDs, NegEIDs, PosEIDsCov, NegEIDsCov,
                         ReducedPosClause, ReducedPosClauseSig, TFPosEIDsCov, TFNegEIDsCov),
  append(ReducedPosClause, NegClause, ReducedClause),
  append(ReducedPosClauseSig, NegClauseSig, ReducedClauseSig),
  full_hypothesis_coverage(ReducedClause, RedcuedClauseSig, PosEIDs, NegEIDs, FPosEIDsCov, FNegEIDsCov).
