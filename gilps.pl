...
:- use_module('engine/par', [runParLgg/1]).
:- use_module('engine/propargolem', [runPropargolem/1]).
:- use_module('bottom clause/bottom_clause', [sat/1, sat/2, ground_sat/1, ground_sat/2]).
...
build_theory(FileName) :-
  setting(engine, parlgg), !,
  runParLgg(FileName).
build_theory(FileName) :-
  setting(engine, propargolem), !,
  runPropargolem(FileName).

% Ability to prespecify pairs

build_theory(FileName, Pairs):-
  setting(engine, toplog), !,
  runTopLog(FileName, Pairs).
build_theory(FileName, Pairs):-
  setting(engine, funclog), !,
  runFuncLog(FileName, Pairs).
build_theory(FileName, Pairs):-
  setting(engine, progolem), !,
  runProgolem(FileName, Pairs).
build_theory(FileName, Pairs) :-
  setting(engine, parlgg), !,
  runParLgg(FileName, Pairs).
build_theory(FileName, Pairs) :-
  setting(engine, propargolem), !,
  runPropargolem(FileName, Pairs).
